...
class RPCSystem(object):
    ...
    def start(self):
        ...
        print("[RPCSystem] RPC 系统已启动，开始 UDP 发现...")
    ...
    def _threadDiscoveryLoop(self):
        ...
        try:
                        ...
                        try:
                            ...
                            if response.startswith(DISCOVERY_RESPONSE_PREFIX):
                                ...
                                if len(parts) >= 3:
                                    ...
                                    # print(serverIp)
                                    # print("[RPCSystem] 发现服务器: " + serverIp + ":" + str(serverPort))
                        except socket.timeout:
                            pass
                    
                    except Exception as e:
                        print("[RPCSystem] UDP 发现异常: " + str(e))
                        # import traceback
                        # traceback.print_exc()
            ...
            print("[RPCSystem] UDP 发现线程异常: " + str(e))
        finally:
            if udp_sock:
                try:
                    udp_sock.close()
                    ...
                    pass
            print("[RPCSystem] UDP 发现线程已退出")
    
    def _threadListenLoop(self):
        """内部线程：TCP 监听循环 - 周期性尝试连接和接收数据"""
        print("[RPCSystem] TCP 监听线程已启动")
            ...
            try:
                ...
                try:
                    ...
                    with self.mLock:
                        self.connected = False
                        self.sock = None
                    print("[RPCSystem] 套接字错误: " + str(e))
            
            except Exception as e:
                print("[RPCSystem] 接收异常: " + str(e))
                with self.mLock:
                    self.connected = False
                    self.sock = None
                time.sleep(0.1)
    ...
    def _tryConnect(self, host, port):
        ...
        try:
            ...
            # print("[RPCSystem] 已连接到调试服务器，地址：%s:%d" % (host, port))
        except socket.error:
            # 连接失败是正常的，会在下一个周期重试
            pass
        except Exception as e:
            # print("[RPCSystem] 连接异常: " + str(e))
            import traceback
            traceback.print_exc()
    ...
    def _sendHandshake(self):
        ...
        try:            
            handshakeData = json.dumps({
                ...
                "platform": RPCSystem.getPlatformInfoName(),
                "name": "Minecraft"
            ...
            try:
                ...
                if handler:
                    try:
                        ...
                        print("[RPCSystem] 处理器异常: " + str(e))
                        import traceback
                        traceback.print_exc()
                else:
                    print("[RPCSystem] 未知的 TypeID 数据包: " + str(typeID))
            
            except Exception as e:
                print("[RPCSystem] 处理数据包异常: " + str(e))
                import traceback
                traceback.print_exc()
                break

# 全局 RPC 系统实例
_RPCSYSTEM = None   # type: RPCSystem | None

def GET_RPC_SYSTEM():
    """获取或创建全局 RPC 系统实例"""
    # type: () -> RPCSystem
    global _RPCSYSTEM
    if _RPCSYSTEM is None:
        _RPCSYSTEM = RPCSystem()
    return _RPCSYSTEM
...
if __name__ == "__main__":
    ...
    if sys.platform == "win32":
        ...
        kernel32.SetConsoleOutputCP(65001)  # UTF-8 编码页
    
    def clientExecCodeHandler(data):
        try:
            print(data)
        except Exception as e:
            print(u"[ClientExec] 处理执行请求异常: " + str(e))
    
    def serverExecCodeHandler(data):
        try:
            print(data)
        except Exception as e:
            print(u"[ServerExec] 处理执行请求异常: " + str(e))

    rpc = GET_RPC_SYSTEM()
    rpc.start()
    rpc.updateHandlers({
        TYPE_EXEC_CLIENT: clientExecCodeHandler,
        TYPE_EXEC_SERVER: serverExecCodeHandler
    })
